...
// Full list of fruits for AI analysis
...
export function renderNutritionPage(root) {
  ...
  function renderTable(data) {
    tbody.innerHTML = data.map(fruit => {
      const knownFruit = fruitData.find(f => f.name === fruit);
      return `
        <tr>
          <td>
            <strong>${fruit}</strong>
          ...
          <td><button class="btn-primary btn-sm" data-fruit="${fruit}">Analizar</button></td>
        </tr>
      `;
    }).join('');
  }
