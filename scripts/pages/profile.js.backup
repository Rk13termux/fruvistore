...
import { getUser, getMyCustomer, upsertCustomer, ensureCustomerExists, updateMyCustomer } from '../services/supabaseService.js';
...
export async function renderProfilePage(root) {
  ...
  let profile = await getMyCustomer();
  if (!profile) {
    try { profile = await ensureCustomerExists(); } catch(_) {}
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await updateMyCustomer({ ...payload });
